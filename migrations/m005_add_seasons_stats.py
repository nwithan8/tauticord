...
class Migration(BaseMigration, ABC):
    ...
    def seasons_libraries_config_section_exists(self, file_path: str) -> bool:
        with open(file_path, 'r') as f:
            ...
            # Otherwise, confirm that the Seasons section is already in the config
            path = ["Stats", "Libraries", "Libraries", [], "Seasons", "Enable"]
            return path_exists_in_yaml(yaml_data=data, path=path)

    def seasons_combined_libraries_config_section_exists(self, file_path: str) -> bool:
        with open(file_path, 'r') as f:
            data = yaml.safe_load(f)
            ...
            # Otherwise, confirm that the Seasons section is already in the config
            path = ["Stats", "Libraries", "CombinedLibraries", [], "Seasons", "Enable"]
            return path_exists_in_yaml(yaml_data=data, path=path)
    ...
    def pre_forward_check(self) -> bool:
        ...
        if all([
            self.seasons_libraries_config_section_exists(file_path=self.old_config_file),
            self.seasons_combined_libraries_config_section_exists(file_path=self.old_config_file)
        ]):
            return False
    ...
    def forward(self):
        self.log("Adding Seasons section to Libraries and CombinedLibraries stats")
        ...
        with open(self.old_config_file, 'r') as f:
            ...
            # Need to add the Seasons section to each library in the Libraries section
            for i, _ in enumerate(stat_libraries or []):  # Will iterate 0 times if stat_libraries is None
                new_config.add(value={
                    ...
                    key_path=["Stats", "Libraries", "Libraries", i, "Seasons"])
            # Need to add the Seasons section to each library in the CombinedLibraries section
            for i, _ in enumerate(
                ...
                new_config.add(value={
                    ...
                    key_path=["Stats", "Libraries", "CombinedLibraries", i, "Seasons"])
    ...
    def post_forward_check(self) -> bool:
        # Make sure the "old" config file has the "Seasons" section now
        return all([
            self.seasons_libraries_config_section_exists(file_path=self.old_config_file),
            self.seasons_combined_libraries_config_section_exists(file_path=self.old_config_file)
        ])
